...
public final class PaillierContext {
		...
		int precision)
	{
		...
		if(precision > modulusBitLength)
			throw new IllegalArgumentException("Precision must be less than or equal to the number of bits in the modulus");
		...
		if(signed) {
			if(fullPrecision) {
				...
				maxEncoded = BigInteger.ONE
					.shiftLeft(precision-1)
	...
	public void checkSameContext(PaillierContext context) throws
	 ...
	 */
	public EncryptedNumber checkSameContext(EncryptedNumber encrypted) throws
		PaillierContextMismatchException
	{
	 ...
	 * different context to <code>this</code>.
	 */
	public EncodedNumber checkSameContext(EncodedNumber encoded) throws
		PaillierContextMismatchException
	{
	...
	public BigInteger decodeBigInteger(EncodedNumber encoded) throws
		DecodeException
	{
		return decode(encoded).decodeBigInteger();
	}
	
	public BigInteger decodeApproximateBigInteger(EncodedNumber encoded) throws
		DecodeException 
	{
	...
	public double decodeApproximateDouble(EncodedNumber encoded) throws
		DecodeException
	{
	...
	public long decodeApproximateLong(EncodedNumber encoded) throws
		DecodeException
	{
	...
	public EncryptedNumber obfuscate(EncryptedNumber encrypted) {
		...
		final BigInteger obfuscated =
			randomPositiveNumber(modulus)
			.modPow(modulus, modulusSquared)
			.multiply(value)
			.mod(modulusSquared);
	...
	public EncryptedNumber add(
		EncryptedNumber operand1,
		EncryptedNumber operand2) throws
		PaillierContextMismatchException
	{
		...
		if(exponent1 > exponent2) {
			value1 = value1.modPow(
				BigInteger.ONE.shiftLeft(exponent1 - exponent2),
				modulusSquared);
			exponent1 = exponent2;
		} else if(exponent1 < exponent2) {
			value2 = value2.modPow(
				BigInteger.ONE.shiftLeft(exponent2 - exponent1),
				modulusSquared);
			exponent2 = exponent1;
		} // else do nothing
...
//				throw new ArithmeticException(); // TODO better ways to detect
	...
	public EncryptedNumber additiveInverse(EncryptedNumber operand1) throws
		PaillierContextMismatchException
	{
		checkSameContext(operand1);
		return new EncryptedNumber(
			operand1.getContext(),
			operand1.ciphertext.modInverse(
				operand1.getContext().getPublicKey().getModulusSquared()),
			operand1.getExponent());
	}
	
	public EncodedNumber additiveInverse(EncodedNumber operand1) throws
		PaillierContextMismatchException
	{
		checkSameContext(operand1);
	...
	public EncryptedNumber subtract(
		EncryptedNumber operand1,
		EncryptedNumber operand2) throws
		PaillierContextMismatchException
	{
